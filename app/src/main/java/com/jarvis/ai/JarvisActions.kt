...
object JarvisActions {
    ...
    fun execute(context: Context, prompt: String): String? {
        ...
        if (lower.startsWith("recuerda que ") || lower.startsWith("acuérdate de ") || lower.startsWith("acuérdate que ")) {
            val memory = text.substringAfter(" ").substringAfter(" ").trim()
        ...
        if (lower.startsWith("llévame a ") || lower.startsWith("llevame a ") || lower.startsWith("cómo llegar a ") || lower.startsWith("como llegar a ")) {
            val destination = lower.substringAfter(" a ").trim()
        ...
        if (lower.startsWith("añade al calendario ") || lower.startsWith("anade al calendario ") || lower.startsWith("crear evento ")) {
            ...
            val intent = Intent(Intent.ACTION_INSERT).setData(CalendarContract.Events.CONTENT_URI).putExtra(CalendarContract.Events.TITLE, title)
        ...
        if (lower == "abre alexa" || lower.contains("controla mi casa") || lower.contains("casa inteligente")) {
            return launchApp(context, "com.amazon.dee.app", "Alexa")
        }
    ...
    private fun launchApp(context: Context, packageName: String, name: String): String {
        ...
        return if (intent != null) {
            ...
            "No encuentro $name instalado en este teléfono."
        }
    }
